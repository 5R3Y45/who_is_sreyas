(function($) {
  ...
  function buildPortfolioItem(project) {
    var groups = JSON.stringify(project.groups);

    return (
      "<figure class=\"item standard\" data-groups='" + groups + "'>" +
        "<div class=\"portfolio-item-img\">" +
          "<img src=\"" + escapeHtml(project.image) + "\" alt=\"" + escapeHtml(project.title) + "\" title=\"\" />" +
        "</div>" +
        "<i class=\"far fa-file-alt\"></i>" +
        "<h4 class=\"name\">" +
          "<a href=\"" + escapeHtml(project.link) + "\" target=\"_blank\" rel=\"noopener noreferrer\">" + escapeHtml(project.title) + "</a>" +
        "</h4>" +
        "<span class=\"category\">" + escapeHtml(project.categoryText) + "</span>" +
      "</figure>"
    );
  }
  ...
  function renderPortfolioProjects() {
    ...
    $.getJSON("data/projects.json")
      .done(function(projects) {
        var portfolioMarkup = projects
          .slice()
          .reverse()
          .map(buildPortfolioItem)
          .join("");

        $portfolioGrid.html(portfolioMarkup);
        deferred.resolve();
      })
      .fail(function() {
        console.error("Unable to load portfolio projects from data/projects.json");
        deferred.reject();
      });
  ...
  $(document).on("ready", function() {
    ...
    renderPortfolioProjects().always(function() {
